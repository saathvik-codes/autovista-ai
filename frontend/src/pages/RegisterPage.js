  ...
  return (
    <motion.div className="dark-page min-h-screen flex items-center justify-center px-6"
      ...
      <motion.div className="relative z-10 w-full max-w-md backdrop-blur-2xl bg-black/50 border border-white/15 rounded-3xl p-8 shadow-2xl"
        ...
        <h1 className="text-3xl sm:text-4xl font-bold mb-2" style={{ fontFamily: "'Outfit', sans-serif" }}>Get Started</h1>
        <p className="text-gray-400 mb-8 text-sm">Create your account to discover your dream car</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <Input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)}
              ...
              required data-testid="name-input" placeholder="John Doe" />
          </div>
          ...
          <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
            <Button type="submit" disabled={loading} data-testid="register-button"
              ...
              {loading ? <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 0.8 }} className="w-5 h-5 border-2 border-white border-t-transparent rounded-full" /> : 'Create Account'}
            </Button>
          </motion.div>
        </form>
        ...
        <p className="text-center text-gray-400 mt-6 text-sm">
          ...
          <Link to="/login" className="text-cyan-400 hover:text-cyan-300 transition-colors font-medium" data-testid="login-link">Login here</Link>
        </p>
      </motion.div>
    </motion.div>
  );
